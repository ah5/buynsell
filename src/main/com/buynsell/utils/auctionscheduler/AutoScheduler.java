...
public class AutoScheduler {
	...
	/*
	 * public void scheduleAuctions() throws Exception { sched.start();
	 * ArrayList auctionsToBeActive=JdbcData.loadAllAuctionsToBeActive();
	 * if(auctionsToBeActive.size() > 0) { for(int i=0;
	 * i<auctionsToBeActive.size(); i++) { Auction
	 * a=(Auction)auctionsToBeActive.get(i); jobDetail=new
	 * JobDetail(a.getAuctionid(), sched.DEFAULT_GROUP, StartAuctionsJob.class);
	 * jobData=new JobDataMap(); jobData.put("auctionID", a.getAuctionid());
	 * jobData.put("catalogID", a.getCatalogid());
	 * jobDetail.setJobDataMap(jobData); triggerJob=new
	 * SimpleTrigger(a.getAuctionid(),sched.DEFAULT_GROUP,
	 * JdbcUtil.fromSQLStringToJavaDate(a.getStartingdate()), null, 0, 0L);
	 * sched.scheduleJob(jobDetail, triggerJob); } } }
	 */

	/*
	 * public void unscheduleAuctions() throws Exception { sched.start();
	 * ArrayList auctionsToBeInActive=JdbcData.loadAllAuctionsToBeInActive();
	 * if(auctionsToBeInActive.size() > 0) { for(int i=0;
	 * i<auctionsToBeInActive.size(); i++) { Auction
	 * a=(Auction)auctionsToBeInActive.get(i);
	 * 
	 * jobDetail=new JobDetail(a.getAuctionid(), sched.DEFAULT_GROUP,
	 * StopAuctionsJob.class); jobData=new JobDataMap();
	 * jobData.put("auctionID", a.getAuctionid()); jobData.put("catalogID",
	 * a.getCatalogid()); jobDetail.setJobDataMap(jobData); triggerJob=new
	 * SimpleTrigger(a.getAuctionid(),sched.DEFAULT_GROUP,
	 * JdbcUtil.fromSQLStringToJavaDate(a.getStartingdate()), null, 0, 0L);
	 * sched.scheduleJob(jobDetail, triggerJob); } } }
	 */
