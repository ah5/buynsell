...
public class CreditCardAddAction extends Action
{
								...
								throws Exception
	{
		...
		if(	Integer.parseInt(ccform.getExpyear()) >= 0 	&&
			Integer.parseInt(ccform.getExpyear()) <= 20)
			yr="20"+ccform.getExpyear();
		else if(	Integer.parseInt(ccform.getExpyear()) >= 97 	&&
					Integer.parseInt(ccform.getExpyear()) <= 99)
		...
		String d=yr+"-"+mo+"-"+da;
		String sql="insert into creditcarddetails values('"+ccform.getUserid()+"','"+
					ccform.getCcnumber()+"','"+ccform.getCctype()+"','"+d+"')";
		JdbcUtil.updtQuery(sql);
		
		ResponseMessage m=new ResponseMessage();
		m.setType("Success");
		m.setHeading("Credit Card Details Added Successful");
