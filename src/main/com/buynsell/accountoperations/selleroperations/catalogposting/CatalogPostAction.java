...
import com.buynsell.utils.auctionscheduler.AutoScheduler;
...
public class CatalogPostAction extends Action {
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (catalog.validator()) {
			...
			String aStDate = catalog.getAuctionstartingdate() + " " + catalog.getShrs() + ":" + catalog.getSmins() + ":"
					+ "00";
			String aEnDate = catalog.getAuctionendingdate() + " " + catalog.getEhrs() + ":" + catalog.getEmins() + ":"
					+ "00";
			...
			query = "insert into product values('" + id + "','" + cID + "','" + name + "','" + desc + "','" + cat
					+ "','" + qty + "','" + wt + "')";
			JdbcUtil.updtQuery(query);

			Auction auction = new Auction();
			auction.setAuctionid(aID);
			auction.setCatalogid(cID);
			auction.setStartingdate(aStDate);
			auction.setEndingdate(aEnDate);
			request.getSession().setAttribute("auction", auction);

			Catalog catalog1 = new Catalog();
			catalog1.setCatalogid(cID);
			catalog1.setUserid(uID);
			catalog1.setStartingprice(stPrc);
			catalog1.setBidincrement(bidIncr);
